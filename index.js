  ...
  try {
    ...
    if (!googleClientId) {
      return res.status(500).json({
        message: "Google OAuth is not configured. Set GOOGLE_CLIENT_ID in driveFleet-server/.env",
      });
    }
...
app.post("/bookings", verifyToken, requireDb, async (req, res) => {
  const { cars, bookings } = getCollections();
    try {
      const { carId, driverNeeded, specialNote, pickupDate, returnDate } = req.body;
      if (!carId || !driverNeeded) {
        return res.status(400).json({ message: "Car and driver preference required" });
      }
